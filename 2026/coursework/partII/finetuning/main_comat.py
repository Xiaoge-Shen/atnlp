"""
main_comat.py — SFT training using the CoMAT-structured prompt format (Q5 Option B).

Mirrors finetuning/main.py exactly, but uses comat_formatting_prompts_func
from prompt_comat.py instead of the generic sft_formatting_prompts_func.

Run:
    python finetuning/main_comat.py \
        --model_signature Qwen/Qwen2.5-0.5B-Instruct \
        --output_path ./checkpoints/Qwen/Qwen2.5-0.5B-Instruct-comat-sft \
...
from prompt_comat import comat_formatting_prompts_func
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SFT training with CoMAT-structured prompt (Q5)")
    parser.add_argument("--model_signature", default="Qwen/Qwen2.5-0.5B-Instruct",
                        help="HuggingFace model signature for training.")
    parser.add_argument("--output_path", default=None, required=True,
                        help="Path to save the adapter.")
    parser.add_argument("--learning_rate", type=float, default=2e-5,
                        help="Learning rate.")
    parser.add_argument("--wandb_project", default="nlu-gsm8k-comat",
    ...
    if args.wandb_token:
        print(f"Logging into WandB with provided token...")
    ...
    def map_to_text(example):
        return comat_formatting_prompts_func(tokenizer, example)
    ...
    training_arguments = get_training_arguments(
        output_model=OUTPUT_DIR, learning_rate=args.learning_rate
    )
